...
# Run the compiled program with input from input.txt and store the output in a temporary file
if [ ! -f "input.txt" ]; then
    echo "Error: input.txt not found. Exiting."
    exit 1
fi

./workspace < input.txt > output.txt
